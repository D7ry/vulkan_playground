...
//                         = { {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, static_cast<uint32_t>(numFrameInFlight)},
//                         {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, static_cast<uint32_t>(numFrameInFlight)},
//                            {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(numFrameInFlight)}};
...
//                         TextureManager::GetSingleton()->GetDescriptorImageInfo(texturePath, descriptorImageInfo);
...
MetaPipeline CreateGenericMetaPipeline(
        ...
        // pipeline
        {
                ...
                if (vkCreateGraphicsPipelines(
                            device->logicalDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m.pipeline
                    )
                    != VK_SUCCESS) {
                        FATAL("Failed to create graphics pipeline!");
                }
