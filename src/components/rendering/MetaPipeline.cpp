...
MetaPipeline CreateGenericMetaPipeline(
    ...
    // descriptor pool
    { // TODO: either dynamically resize descriptor pool on render group
      // creation, or allocate a fat pool at start.
        ...
        if (vkCreateDescriptorPool(
                device->logicalDevice, &poolInfo, nullptr, &m.descriptorPool
            )
            != VK_SUCCESS) {
...

    {
        ...
        VkShaderModule vertShaderModule = ShaderCreation::createShaderModule(
            device->logicalDevice, vertexShader.data()
        );
        VkShaderModule fragShaderModule = ShaderCreation::createShaderModule(
            device->logicalDevice, fragmentShader.data()
        );
        ...
        inputAssembly.primitiveRestartEnable
            = VK_FALSE; // don't restart primitives
        ...
        pipelineLayoutInfo.pSetLayouts
            = &m.descriptorSetLayout;                     // bind our own layout
